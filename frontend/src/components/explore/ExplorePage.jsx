...
// How long (ms) to wait after an interaction before re-fetching recommendations.
// Debouncing prevents hammering the backend when a user rapidly interacts.
const RECO_DEBOUNCE_MS = 800;

const ExplorePage = ({ user }) => {
  const navigate = useNavigate();
  const userRef = useRef(user);
  const recoTimerRef = useRef(null);

  const [articles, setArticles] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [activeCategory, setActiveCategory] = useState("All");
  const [loading, setLoading] = useState(false);
  const [recoRefreshing, setRecoRefreshing] = useState(false); // subtle refresh indicator
  ...
  const fetchUserInteractions = useCallback(async (userId) => {
    try {
  ...
  const fetchRecommendations = useCallback(async (userId, { silent = false } = {}) => {
    if (!userId) return;
    if (!silent) setLoading(true);
    else setRecoRefreshing(true);
    setError(null);
    try {
  ...
  }, [user?.id, fetchUserInteractions, fetchRecommendations]);

  // ── Debounced reco refresh ──────────────────────────────────────────────
  const scheduleRecoRefresh = useCallback((userId) => {
    if (recoTimerRef.current) clearTimeout(recoTimerRef.current);
    recoTimerRef.current = setTimeout(() => {
      fetchRecommendations(userId, { silent: true });
    }, RECO_DEBOUNCE_MS);
  }, [fetchRecommendations]);

  // Cleanup on unmount
  useEffect(() => () => { if (recoTimerRef.current) clearTimeout(recoTimerRef.current); }, []);
  ...
  const handleArticleInteraction = async (articleId, interactionType) => {
    ...
    try {
      ...
      // Every meaningful interaction re-scores recommendations (debounced)
      const RECO_TRIGGERS = ["like", "unlike", "dislike", "save", "unsave", "share"];
      if (RECO_TRIGGERS.includes(interactionType)) {
        scheduleRecoRefresh(currentUser.id);
      }
    } catch (err) {
    ...
    <div className="explore-page">
      <div className="explore-header">
        <div className="explore-header-row">
          <h1>Explore</h1>
          {recoRefreshing && (
            <span className="reco-refreshing-badge">✨ Updating feed…</span>
          )}
        </div>
      </div>
